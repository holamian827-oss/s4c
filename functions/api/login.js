...
export async function onRequestPost({ request, env }) {
    try {
        ...
        // 2. 智能補全：判斷輸入的是不是純數字 (如 27) 或是純姓名 (如 羅珏俊)
        if (!username.includes('_')) {
            let entry = null;
            if (!isNaN(username) && username.length > 0) {
                // 輸入數字
                entry = await env.DB.prepare("SELECT username FROM users WHERE username LIKE ? AND role = 'student'")
            ...
            if (entry) username = entry.username; // 成功匹配，賦值為 "27_羅珏俊"
        }
